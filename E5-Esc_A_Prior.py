...
def evaluar_escalabilidad_prioridades(device_type="cpu", num_samples=10000):
    # Cargar modelo y tokenizer
    try:
        model = AutoModelForSequenceClassification.from_pretrained(os.path.join(MODEL_DIR, "best_model"))
        tokenizer = AutoTokenizer.from_pretrained(os.path.join(MODEL_DIR, "best_model"))
    ...
    try:
        df = pd.read_csv(os.path.join(DATA_DIR, "crisis_limpio.csv")).sample(num_samples)
    ...
    for batch_size in BATCH_SIZES:
        for text_size in TEXT_SIZES:
            ...
            num_batches = max(1, len(sample_texts) // batch_size)  # Asegurar al menos 1 lote
            processing_times = []
            cpu_usage = []
            gpu_memory = []
            ...
            try:
                for i in tqdm(range(num_batches), desc=f"Procesando {batch_size}-batch"):
                    ...
                    if not batch_texts:  # Evitar lotes vacíos
                        continue
            ...
            texts_per_second = text_size / total_time if total_time > 0 else np.nan
...
def plot_escalabilidad(results):
    ...
    plt.suptitle('Comparación de Escalabilidad: CPU vs GPU (Modelo de Prioridades)', fontsize=16)
...
if __name__ == "__main__":
    # Probar con CPU
    cpu_results = evaluar_escalabilidad_prioridades(device_type="cpu")
    
    # Probar con GPU si está disponible
    gpu_results = pd.DataFrame()
    if torch.cuda.is_available():
        gpu_results = evaluar_escalabilidad_prioridades(device_type="cuda")
    ...
    if not results.empty:
        results.to_csv("escalabilidad_prioridades.csv", index=False)
        print("\n✅ Resultados guardados en 'escalabilidad_prioridades.csv'")
        print(results)
